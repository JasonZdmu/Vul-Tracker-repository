...
class EnsembleModel(nn.Module):
    def __init__(self, ablation_study=False, variant_to_drop=None, num_layers=2, nhead=4):
        ...
        """CNN 模块"""
        self.cnn_1 = nn.Conv1d(in_channels=1, out_channels=32, kernel_size=3, padding=1)
        self.cnn_2 = nn.Conv1d(in_channels=1, out_channels=32, kernel_size=3, padding=1)
        ...
        self.l5 = nn.Linear(112 * self.FEATURE_DIM, self.FEATURE_DIM) if not ablation_study else nn.Linear((7 - len(variant_to_drop)) * self.FEATURE_DIM, self.FEATURE_DIM)
    ...
    def forward(self, feature_1, feature_2, feature_3, feature_5, feature_6, feature_7, feature_8, need_features=False):
        ...
        feature_1 = feature_1.unsqueeze(1)  # CNN 期望输入形状为 (batch_size, channels, width)
        ...
        """CNN 处理"""
        feature_set_1 = torch.cat([feature_1, feature_2, feature_3], axis=2)
        feature_set_2 = torch.cat([feature_5, feature_6, feature_7, feature_8], axis=2)

        cnn_out_1 = self.cnn_pool(self.relu(self.cnn_1(feature_set_1)))
        cnn_out_2 = self.cnn_pool(self.relu(self.cnn_2(feature_set_2)))
        print("cnn_out_1的shape: ", cnn_out_1.shape)
        print("cnn_out_2的shape: ", cnn_out_2.shape)


        """特征合并"""
        combined_features = torch.cat([cnn_out_1, cnn_out_2], axis=2)
        print("拼接以后combined_features的shape: ", cnn_out_1.shape)
        combined_features = combined_features.flatten(start_dim=1)  # Flatten for linear layer
        print("展平以后combined_features的shape: ", cnn_out_1.shape)

        combined_features = self.drop_out(combined_features)
        print("最终的combined_features的shape: ", combined_features.shape)

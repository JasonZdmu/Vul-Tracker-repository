...
from model_xiao import EnsembleModel
import torch
...
def setup_experiment(experiment_type):
    ...
    if experiment_type == "line":
        return [1, 2, 3, 5, 6, 7]  # 只保留 feature_8
    elif experiment_type == "hunk":
        return [1, 2, 5, 6, 8]  # 只保留 feature_3 和 feature_7
    elif experiment_type == "file":
        return [1, 3, 5, 7, 8]  # 只保留 feature_2 和 feature_6
    elif experiment_type == "commit":
        return [2, 3, 6, 7, 8]  # 只保留 feature_1 和 feature_5
    elif experiment_type == "context_dependent":
        return [5, 6, 7, 8]  # 只保留 feature_1, feature_2, feature_3
    elif experiment_type == "context_free":
        return [1, 2, 3]  # 只保留 feature_5, feature_6, feature_7, feature_8
    else:
        return []  # 默认不丢弃任何特征
...
def do_train(args, variant_to_drop):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_type', type=str, required=True, help='Type of experiment (line, hunk, file, commit, context_dependent, context_free)')
    ...
    # 根据实验类型设置要丢弃的特征
    variant_to_drop = setup_experiment(args.experiment_type)
    do_train(args, variant_to_drop)

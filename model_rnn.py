...
class EnsembleModel(nn.Module):
    def __init__(self, ablation_study=False, variant_to_drop=None, num_layers=2, nhead=4):
        ...
        """RNN 模块"""
        self.rnn_1 = nn.RNN(input_size=3 * self.FEATURE_DIM, hidden_size=self.FEATURE_DIM, num_layers=num_layers, batch_first=True)
        self.rnn_2 = nn.RNN(input_size=4 * self.FEATURE_DIM, hidden_size=self.FEATURE_DIM, num_layers=num_layers, batch_first=True)
        ...
        self.l5 = nn.Linear(2 * self.FEATURE_DIM, self.FEATURE_DIM) if not ablation_study else nn.Linear((7 - len(variant_to_drop)) * self.FEATURE_DIM, self.FEATURE_DIM)
    ...
    def forward(self, feature_1, feature_2, feature_3, feature_5, feature_6, feature_7, feature_8, need_features=False):
        ...
        """RNN 处理"""
        feature_set_1 = torch.cat([feature_1, feature_2, feature_3], axis=2)  # shape: [batch_size, seq_len, 3 * FEATURE_DIM]
        feature_set_2 = torch.cat([feature_5, feature_6, feature_7, feature_8], axis=2)  # shape: [batch_size, seq_len, 4 * FEATURE_DIM]

        rnn_out_1, _ = self.rnn_1(feature_set_1)
        rnn_out_2, _ = self.rnn_2(feature_set_2)

        """特征合并"""
        combined_features = torch.cat([rnn_out_1, rnn_out_2], axis=2)
        print("拼接后的combined_features的shape",combined_features.shape)

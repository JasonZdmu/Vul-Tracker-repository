...
def train(model, learning_rate, number_of_epochs, training_generator, gold_loader, test_java_generator,
          test_python_generator, eval_interval=EVAL_INTERVAL):
    ...
    for epoch in range(number_of_epochs):
        ...
        # Phase 2: Train with corrected loss
...
def do_train(args):
    ...
    gold_loader = DataLoader(training_set, batch_size=VALIDATION_BATCH_SIZE, shuffle=False, num_workers=8)

    train(model=model, learning_rate=LEARNING_RATE, number_of_epochs=NUMBER_OF_EPOCHS,
          training_generator=training_generator, gold_loader=gold_loader, test_java_generator=test_java_generator,

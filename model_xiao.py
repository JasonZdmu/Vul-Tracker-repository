...
class EnsembleModel(nn.Module):
    def __init__(self, ablation_study=False, variant_to_drop=None, num_layers=2, nhead=4):
        ...
        self.FEATURE_DIM = 768  # 基础特征维度
        self.DENSE_DIM = 128  # DENSE特征维度
        self.CNN_FEATURE_DIM = 300  # CNN特征维度
        self.HIDDEN_DIM_DROPOUT_PROB = 0.5  # Dropout 概率
        self.NUMBER_OF_LABELS = 2  # 二分类问题

        # 定义线性层，分别处理不同类型的特征
        self.l1 = nn.Linear(self.CNN_FEATURE_DIM, self.FEATURE_DIM)  # Hunk (Context-dependent)
        self.l2 = nn.Linear(self.CNN_FEATURE_DIM * 2, self.FEATURE_DIM)  # Hunk (Context-free)
        self.l3 = nn.Linear(self.DENSE_DIM, self.FEATURE_DIM)  # Commit (Context-free)
        self.l4 = nn.Linear(self.DENSE_DIM, self.FEATURE_DIM)  # Line (Context-free)

        # 动态创建 Transformer 编码器
        self.nhead = nhead
        self.num_layers = num_layers
        self.ablation_study = ablation_study
        self.variant_to_drop = variant_to_drop or []

        # 动态调整 l5 层的输入维度
        num_active_features = 7 - len(variant_to_drop)  # 7个特征，减去被丢弃的特征
        self.l5 = nn.Linear(num_active_features * self.FEATURE_DIM, self.FEATURE_DIM)

        self.relu = nn.ReLU()
        self.drop_out = nn.Dropout(self.HIDDEN_DIM_DROPOUT_PROB)
        self.out_proj = nn.Linear(self.FEATURE_DIM, self.NUMBER_OF_LABELS)

    def create_transformer_encoder(self, input_dim):
        """
        动态创建 Transformer 编码器，自动调整 d_model。
        """
        return TransformerEncoder(
            TransformerEncoderLayer(d_model=input_dim, nhead=self.nhead),
            num_layers=self.num_layers
        )

    def forward(self, feature_1, feature_2, feature_3, feature_5, feature_6, feature_7, feature_8, need_features=False):
        """
        Forward 函数，动态处理输入特征，结合 context-dependent 和 context-free 特征。
        """

        # context-dependent 和 context-free 特征列表
        feature_list_1 = []  # Context-dependent
        feature_list_2 = []  # Context-free

        # 动态选择特定特征，只有在 variant_to_drop 中不包含的特征才会被保留
        if 1 not in self.variant_to_drop:  # Commit (Context-dependent)
            feature_1 = feature_1.unsqueeze(1).to(device)
            feature_list_1.append(feature_1)

        if 2 not in self.variant_to_drop:  # File (Context-dependent)
            feature_2 = feature_2.unsqueeze(1).to(device)
            feature_list_1.append(feature_2)

        if 3 not in self.variant_to_drop:  # Hunk (Context-dependent)
            feature_3 = self.l1(feature_3).unsqueeze(1).to(device)
            feature_list_1.append(feature_3)

        if 5 not in self.variant_to_drop:  # Commit (Context-free)
            feature_5 = self.l3(feature_5).unsqueeze(1).to(device)
            feature_list_2.append(feature_5)

        if 6 not in self.variant_to_drop:  # File (Context-free)
            feature_6 = feature_6.unsqueeze(1).to(device)
            feature_list_2.append(feature_6)

        if 7 not in self.variant_to_drop:  # Hunk (Context-free)
            feature_7 = self.l2(feature_7).unsqueeze(1).to(device)
            feature_list_2.append(feature_7)

        if 8 not in self.variant_to_drop:  # Line (Context-free)
            feature_8 = self.l4(feature_8).unsqueeze(1).to(device)
            feature_list_2.append(feature_8)

        # 将 context-dependent 特征组合并通过 transformer 处理
        if len(feature_list_1) > 0:
            feature_set_1 = torch.cat(feature_list_1, axis=2)
            feature_set_1 = feature_set_1.to(device)  # 确保特征在 GPU 上
            transformer_encoder_1 = self.create_transformer_encoder(input_dim=feature_set_1.shape[2])
            transformer_encoder_1 = transformer_encoder_1.to(device)  # 确保 transformer_encoder 在 GPU 上
            transformed_features_1 = transformer_encoder_1(feature_set_1)
        else:
            transformed_features_1 = None

        # 将 context-free 特征组合并通过 transformer 处理
        if len(feature_list_2) > 0:
            feature_set_2 = torch.cat(feature_list_2, axis=2)
            feature_set_2 = feature_set_2.to(device)  # 确保特征在 GPU 上
            transformer_encoder_2 = self.create_transformer_encoder(input_dim=feature_set_2.shape[2])
            transformer_encoder_2 = transformer_encoder_2.to(device)  # 确保 transformer_encoder 在 GPU 上
            transformed_features_2 = transformer_encoder_2(feature_set_2)
        else:
            transformed_features_2 = None

        # 合并处理后的特征
        if transformed_features_1 is not None and transformed_features_2 is not None:
            combined_features = torch.cat([transformed_features_1, transformed_features_2], axis=2)
        elif transformed_features_1 is not None:
            combined_features = transformed_features_1
        else:
            combined_features = transformed_features_2

        print("Shape of combined_features before l5:", combined_features.shape)
